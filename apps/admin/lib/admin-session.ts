...
export function sessionConfigured(){return Boolean(secret()&&process.env.ADMIN_DEMO_EMAIL?.trim()&&process.env.ADMIN_DEMO_PASSWORD?.trim())}
export function credentialsValid(email:string,password:string){const expectedEmail=process.env.ADMIN_DEMO_EMAIL?.trim().toLowerCase()??'',expectedPassword=process.env.ADMIN_DEMO_PASSWORD??'';const given=Buffer.from(password),expected=Buffer.from(expectedPassword);return email.trim().toLowerCase()===expectedEmail&&given.length===expected.length&&timingSafeEqual(given,expected)}
